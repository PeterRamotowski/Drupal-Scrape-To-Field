...
- [Feeds](https://www.drupal.org/project/feeds) - Import and aggregate content from various sources. Feeds works on node level, each node has the same source URL and configuration. **Scrape to field** works on field level, each field on each node can have different source URL and configuration.

- [Migrate](https://www.drupal.org/project/migrate_plus) - A powerful framework for migrating data into Drupal from various sources. **Migrate** requires custom migration configuration files while **Scrape to field** uses a GUI for setup.
